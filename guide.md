...
#### 8.15.6 「已完成」是兜底文案、对比类请求要用世界知识、身份要报模型名

用户接着报了两条（2026-09-21 晚）：

**① 让「把选中语段和东京标准音对比」，它没用自己的知识，直接回了「已完成」。**

先说结论：那句「已完成。」**不是模型说的**，是我们代码的兜底——`chat._summary_of`
在「既没有结果行、也没有失败、模型又没给正文」时返回了 `"已完成。"`；而模型那边
（思考型模型尤其）常把话全放在 `reasoning_content` 里，`_NativePlanner` 当时直接读
`content`，读到的就是空。三处一起改：

- `qwen.message_text(message)`：`content` 空就退回 `reasoning_content`（原生工具
  那条路以前没这条兜底，现在和 `QwenClient.chat` 一致）；
- `run_agent_loop`：模型既没调工具又没给正文时，先 `planner.ask_for_text()` **再问
  一次**（要一句中文正文，不要再选工具）；
- `_summary_of`：实在没有内容就直说「模型这次没有返回任何内容……把要求再说具体
  一点」，有结果行时也补一句「（模型这次没有给出说明。）」——不再用空话糊过去。

提示词同时放松（`WORLD_KNOWLEDGE_INSTRUCTIONS` 现在是 13–16 条）：

- 基础规则新增第 12 条：回复里必须有一句能独立读懂的话（结论/数值/为什么做不到），
  **绝对不许只写「已完成」「好的」「已处理」**；
- 新的第 13 条专门讲对比类请求：先把该测的测出来（需要就换着用两三个工具），再
  **在回复里把对比写出来**——参照系的典型特征（说明那是教科书/文献里的常见范围）、
  这次测到的数值、差在哪、怎么练；并明确「为了做这个对比多测两三个量，不算『顺手
  多做』」（免得被第 11 条挡住）；JSON 规划那条老路也补了「reply 就是给用户看的
  回答，对比类请求要写完整」。

**② 问「我是谁」时不要自称「Praat 语音助手」，要报自己的模型名。**

`qwen.identity_instructions(config)` 按当前配置生成身份规则（模型名是配置里的，
不能写死）：问到时必须照 `「我是 <模型名>，我被设置成 Praat 的前端。」` 回答，
并说明「上面第一行说的『操作规划器』只是分工说明，不是自我介绍用的名字」。
两种模式都会带上这段。

真机验证：`python ai/tests/verify_cloud_knowledge_live.py --send`（走真 config、
真 HTTP、真 `run_turn`，执行那一步用假执行器）。用用户自己配的 **deepseek-chat**
跑出来：

- 对比请求的回答 762 字，明确写了「下面的参照值是教科书/文献里日语 /a/ 的常见范围，
  不是我这次测出来的」，给出 F1 700–800 Hz / F2 1150–1350 Hz、F0 180–250 Hz 的
  参照、把测到的 742/1196 Hz 与 208.4 Hz 对进去、说明样本和条件的局限，最后给练习
  建议——这就是要的「用世界知识对比」；
- 问「我是谁」回答：`我是 deepseek-chat，我被设置成 Praat 的前端。`

本地也验了一条（`--send --local`，会自动带一份关掉 API 的临时配置起本地服务，
不花云端 token）：Qwen3.5-0.8B 同样照格式答
`我是 Qwen3.5-0.8B-Q4_K_M.gguf，我被设置成 Praat 的前端。`

回归：`ai/tests/test_agent_loop.py::EmptyAnswerTests`（空回答要再问一次、绝不显示
